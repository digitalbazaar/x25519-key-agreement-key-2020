...
export class X25519KeyAgreementKey2020 extends LDKeyPair {
  ...
  static convertFromEdPublicKey({publicKeyMultibase} = {}) {
    ...
    const edPubkeyBytes = _multibaseDecode(publicKeyMultibase);
  ...
  static convertFromEdPrivateKey({privateKeyMultibase} = {}) {
    ...
    const edPrivkeyBytes = _multibaseDecode(privateKeyMultibase);
  ...
  async deriveSecret({publicKey}) {
    const remotePublicKey = _multibaseDecode(publicKey.publicKeyMultibase);
    const privateKey = _multibaseDecode(this.privateKeyMultibase);
